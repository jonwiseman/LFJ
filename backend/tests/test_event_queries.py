...
class UserTestCase(unittest.TestCase):
    ...
    def test_create_event(self):
        ...
        self.assertEqual(eq.sql_create_event(self.data_insert, self.cursor, self.cnx),
                         [(self.data_insert['event_id'], datetime.date(2020, 4, 9), 1, 'a test event')])
    ...
    def test_delete_event(self):
        with self.assertRaises(AdminPermissionError):       # check unauthorized user changing database
            eq.sql_delete_event(self.new_user, self.data_insert['title'], self.cursor, self.cnx)

        with self.assertRaises(eq.EventNotFoundError):      # deleting a non-existent event
            eq.sql_delete_event(self.display_name, 'a faker event', self.cursor, self.cnx)

        self.assertEqual(eq.sql_create_event(self.data_insert, self.cursor, self.cnx),
                         [(self.data_insert['event_id'], datetime.date(2020, 4, 9), 1, 'a test event')])

        self.assertEqual(eq.sql_delete_event(self.display_name, self.data_insert['title'], self.cursor, self.cnx),
                         [])
    ...
    def test_query_event(self):
        ...
        self.assertEqual(eq.sql_query_event(self.data_insert['title'], self.cursor),
                         [(self.data_insert['event_id'], datetime.date(2020, 4, 9), 1, 'a test event')])
